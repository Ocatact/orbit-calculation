...
function dTdt(T, epsilon, sigma, T_out, C, S, R, gamma, alpha, V, M, p) {
    return ( (epsilon.times(
                sigma.times(
                    T_out.toPower(new Decimal(4))
                    .minus(
                        (T.times(alpha)).toPower(new Decimal(4))
                    )
                )
            ).plus(
                C.times(T_out.minus(T.times(alpha)))
            )
...
async function main() {
    ...
    let alpha = new Decimal(document.getElementById("alpha").value !== "" ? document.getElementById("alpha").value : 0); // 表面温度補正(無次元)
    ...
    while (t.toNumber() <= tmax.toNumber()) {
        ...
        k0[1] = dt.times(dTdt(T, epsilon, sigma, T_out, C, S, R, gamma, alpha, V, M, p));
        ...
        k1[1] = dt.times(dTdt(T.plus(k0[1].div(new Decimal(2))), epsilon, sigma, T_out, C, S,  R, gamma, alpha, V, M, p));
        ...
        k2[1] = dt.times(dTdt(T.plus(k1[1].div(new Decimal(2))), epsilon, sigma, T_out, C, S,  R, gamma, alpha, V, M, p));
        
        k3[0] = dt.times(dvdt(T.plus(k2[1]), v.plus(k2[0]), rho_out, V, g, m, M, p, R, k));
        k3[1] = dt.times(dTdt(T.plus(k2[1]), epsilon, sigma, T_out, C, S,  R, gamma, alpha, V, M, p));
